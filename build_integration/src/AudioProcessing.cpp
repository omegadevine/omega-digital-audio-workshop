...
bool ConvolutionReverb::loadImpulseResponse(const std::string& filename) {
    ...
    std::cout << "Loading impulse response from " << filename << std::endl;
    return false;
}

void ConvolutionReverb::setImpulseResponse(const float* ir, int irLength, int numChannels) {
    impulseResponse_.resize(numChannels);
    inputHistory_.resize(numChannels);
    
    for (int ch = 0; ch < numChannels; ++ch) {
        impulseResponse_[ch].assign(ir, ir + irLength);
        inputHistory_[ch].resize(irLength, 0.0f);
    }
...
void ConvolutionReverb::process(float** inputs, float** outputs, int numChannels, int numFrames) {
    if (impulseResponse_.empty() || !inputs) {
        return;
    }
    
    for (int ch = 0; ch < std::min(numChannels, (int)impulseResponse_.size()); ++ch) {
        convolve(inputs[ch], outputs[ch], numFrames, ch);
    }
}

void ConvolutionReverb::convolve(const float* input, float* output, int numFrames, int channel) {
    const int irLength = impulseResponse_[channel].size();
    
    for (int i = 0; i < numFrames; ++i) {
        // Add input to history
        inputHistory_[channel][historyPos_] = input[i];
        
        // Convolve
        float wet = 0.0f;
        for (int j = 0; j < irLength; ++j) {
            int histIdx = (historyPos_ - j + irLength) % irLength;
            wet += inputHistory_[channel][histIdx] * impulseResponse_[channel][j];
        }
        
        // Mix wet and dry
        output[i] = input[i] * (1.0f - wetDryMix_) + wet * wetDryMix_;
        
        historyPos_ = (historyPos_ + 1) % irLength;
    }
}
